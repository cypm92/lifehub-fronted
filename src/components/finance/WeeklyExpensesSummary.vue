...
const planned = computed(() =>
  props.weeklyExpenses.reduce((total, item) => total + item.allocated_amount, 0)
)
const paid = computed(() =>
...
const weeks = computed(() => {
  ...
  props.weeklyExpenses.forEach((item) =>
    groups.set(item.week_number, [...(groups.get(item.week_number) || []), item])
  )
      ...
      <div v-if="showWeeks !== false" class="weeks-list">
        <div v-for="week in weeks" :key="week.number" class="week-item">
          ...
          ><span
            ...
            ><small>{{ week.label }}</small></span
          ><strong>{{ week.total.toFixed(2) }} €</strong>
        </div>
        <span v-if="!weeks.length" class="empty">Sin semanas configuradas.</span>
      </div>
    </div>
  </section>
</template>
...
.week-item span:nth-child(2) {
  display: flex;
  flex-direction: column;
  min-width: 0;
}
.week-item strong:last-child {
  margin-left: auto;
  white-space: nowrap;
}
.week-state {
